...
keyboard_dice = [
    ['шестигранный', 'два шестигранных одновременно'], 
    ['двадцатигранный', 'Вернуться назад']
]
...
def dice(update, context):
    ...
    update.message.reply_text('Какая кость вам нужна?', reply_markup=keyboard)

def timer(update, context):
    keyboard = ReplyKeyboardMarkup(keyboard_timer, resize_keyboard=True)
    update.message.reply_text('Какое время вам нужно?', reply_markup=keyboard)
...
def task(context):
    job = context.job
    context.bot.send_message(job.context, text='Ваше время истекло')

def choice(update, context):
    text = update.message.text
    if text == 'Вернуться назад':
        start(update, context)
    elif text == 'шестигранный':
        update.message.reply_text(dice_x(6, 1))
    elif text == 'два шестигранных одновременно':
        update.message.reply_text(dice_x(6, 2))
    elif text == 'двадцатигранный':
